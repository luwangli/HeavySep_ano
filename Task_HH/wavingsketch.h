...
public:
	...
	WavingSketch2(int total_memory){
	    BUCKET_NUM = floor(total_memory/sizeof(Bucket));
	    buckets = new Bucket[BUCKET_NUM];
...
*/
	 ...
	 void Print_basic_info()
    {
        ...
        cout<<"\t bucket number: "<<BUCKET_NUM<<"\t slot number: "<<slot_num<<endl;
    ...
    map<uint32_t,int> Query_threshold(int threshold){
        ...
        for(uint32_t i = 0;i<BUCKET_NUM;i++){
            for(int j=0;j<slot_num;j++){
                if(std::abs(buckets[i].counters[j]) > threshold){
                    result.insert(make_pair(buckets[i].items[j],std::abs(buckets[i].counters[j])));
                }
            }
        }
        return result;
